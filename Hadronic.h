...
// Mon Sep 26 13:55:20 2016 by ROOT version 5.34/36
// from TTree Delphes/Analysis tree
// found on file: ttDM_Mchi1Mphi100.root
//////////////////////////////////////////////////////////
...
public :
   ...
   Hadronic(std::string, TTree *tree=0);
   virtual ~Hadronic();
...
#ifdef Hadronic_cxx
Hadronic::Hadronic(std::string _sample, TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   sample = _sample;
   if (tree == 0) {
     TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject((sample+".root").c_str());
      if (!f || !f->IsOpen()) {
	//f = new TFile(("/mnt/t3nfs01/data01/shome/dpinna/Delphes-3.3.3/"+sample+".root").c_str()); 
	f = new TFile(("./rootfiles/"+sample+".root").c_str());
      }
...
Int_t Hadronic::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Hadronic::LoadTree(Long64_t entry)
{
...
void Hadronic::Show(Long64_t entry)
{
...
Int_t Hadronic::Cut(Long64_t entry)
{
